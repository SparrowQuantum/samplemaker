...
# a device and its port map:
# NetListEntry(devicename, x_position,y_position, rotation, portmap, parameters)

# The port map is a python dictionary that assigns each port in the device to a
# port name in the circuit.

dcpl_portmap = {"p1": "inA", "p4": "inB", "p3": "AAA", "p2": "BBB"}
entry_list = [
    smdev.NetListEntry("BASELIB_DCPL", 0, 0, "E", dcpl_portmap, {}),
    smdev.NetListEntry("BASELIB_FGC", -30, 0, "W", {"p1": "inA"}, {}),
    smdev.NetListEntry("BASELIB_FGC", 30, -15, "S", {"p1": "inB"}, {}),
]

# in the above netlist we included our DC and a grating coupler and connected left_top
# to in via inA. We have not connected AAA and BBB.
...
l_portmap = {"AAA": "input", "BBB": "link"}
r_portmap = {"AAA": "link", "BBB": "output"}
elist = [
    smdev.NetListEntry("X", 0, 0, "E", l_portmap, {"NETLIST": netlist}),
    smdev.NetListEntry("X", 40, 12, "E", r_portmap, {"NETLIST": netlist}),
]
# So we placed two of the above netlists and connected BBB of the first to AAA of the
# second
...
###############################################################################
# We can also use circuits (or any port device) in tables and autolink elements between
# rows or columns. Let's see how.
