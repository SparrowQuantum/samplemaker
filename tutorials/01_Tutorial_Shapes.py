...
g = sm.make_rect(0, 2, 6, 3, numkey=3, layer=2)

# Circles:
# A circle in (10,0) with radius 3 um
g += sm.make_circle(10, 0, 3)
...
g += sm.make_circle(20, 0, 3, layer=1, to_poly=True, vertices=50)


# Polygons:
# Arbitrary shaped polygons (but avoid self-intersections)
g += sm.make_poly([30, 40, 35, 31], [0, -2, 8, 7])
# Path:
# Open piece-wise paths with a width
g += sm.make_path([50, 50, 60], [0, 10, 10], 1, to_poly=True)
...
# Text:
g += sm.make_text(70, 0, "Hello", 10, 1, numkey=1, to_poly=True)
...
# Rounded rectangle
g += sm.make_rounded_rect(0, 20, 10, 5, 1, resolution=8)
# Creates a rectangle with 1 um radius rounded edges.

# Ellipse
g += sm.make_ellipse(15, 20, 3, 2, 45, to_poly=True)
...
# Rings
g += sm.make_ring(30, 20, 5, 5, 0, 0.5, to_poly=True)
...
# Arcs
g += sm.make_arc(40, 20, 5, 5, 0, 0.5, 0, 90, to_poly=True)
# Same as ring, but you can specify an initial and a final angle to only draw a sector
# (here from 0 to 90 degrees).
# Can be also elliptical arcs.

# Width-variable paths
g += sm.make_tapered_path([50, 55, 60], [15, 20, 23], [1, 0.5, 2])
...
# This is all you can do with basic samplemaker makers, we are only missing SREF and
