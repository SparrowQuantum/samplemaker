...
# here we use a derived class that can actually draw waveguides according to a specific
# process. Anyone can write their own sequencer instructions for its system.
...
# w -> width at the end
# STORED -> a list of coordinates stored along the sequence, you need to use the "STORE"
# command in the sequence.
# __XC__/__YC__-> position of the waveguide start point relative to the center when
...
# We want to store the position after the first bend. We also want to make the position
# before the second bend at coordinate [0,0]. Finally, in the last command we insert a
# device at the end of the sequence. We specify the input port 'p1' and output port to
# be the same port. More info about devices with port on the next tutorial.
sequencer = BaseWaveguideSequencer(seq)
g4 = sequencer.run()
print(sequencer.state)
