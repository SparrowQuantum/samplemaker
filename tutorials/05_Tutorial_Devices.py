...
# It allows us to create library parts that can be interconnected,
# iterated into tables, etc..
...
# We will take as example a device called free-free membrane 10.1103/PhysRevB.98.155316.
# We draw a rectangular membrane supported by tethers and use boolean operations to
# create the negative to be etched. Instead of typing all code in the main script, we
# create a parametric device by deriving from the base class Device, which we imported
# earlier.
...
class FreeFreeMembrane(Device):
    ...
    def initialize(self):
        ...
        self.set_description(
            "Free free membrane as in 10.1103/PhysRevB.98.155316, etc etc"
        )

    def parameters(self):
        # define all the parameters of the device and their default values.
        # You can specify what type the parameter has and what it the minimum-maximum
        # allowed values. Default is float and range (0,infinity) for all parameters.
        self.addparameter(
            "L", 40, "Length of the membrane", param_type=float, param_range=(0.5, 150)
        )
    ...
    def geom(self):
        ...
        # Note that you can choose whetner a type cast should be made (i.e. forcing the
        # parameter to be of the type specified in the addparameter command) and if it
        # should be clipped in the allowed range.
...
# That's all, we now have a device that we can instantiate multiple times with
# differerent parameters!
...
g += ffm_dev.run()
# TADA! Note that the device has been automatically placed into a SREF, so you can reuse
# it in your code.
# In the next tutorial we will see how to organize devices in collections and
