...
# connectors of different kind. Here we look at the electrical connectors combined with
# optical connectors. We also look how to re-use devices in other devices!
...
ElectricalConnectorOptions = {"elbow_offset": 5, "metal_layer": 3}
...
class ElectricalPort(DevicePort):
    def __init__(
        ...
        self.width = width
        self.name = name
        self.connector_function = connect_electrical_ports


# we want to reuse the FreeFreeMembrane in a device that has a local electrical
# connection, so we make a new device
class FreeFreeMembraneELE(FreeFreeMembrane):
    ...
    def geom(self):
        ...
        mesa_w = self._p["mesawidth"]
        # Draw the mesa area as a filled polygon, here a simple square
        mesa = sm.make_rect(0, 0, mesa_w, mesa_w)

        conn_w = 0.8
        port1 = ElectricalPort(0, mesa_w / 2, "north", conn_w, "emesa")
        # and add it to local port
        self.addlocalport(port1)

        # Now draw a metal pad and wire to the port (again we use fixed values,
        # could be parametrized)
        pad = sm.make_rounded_rect(0, mesa_w / 2 - 5, mesa_w - 10, 4, 1)
        pad += sm.make_path([0, 0], [mesa_w / 2 - 5, mesa_w / 2], conn_w, to_poly=True)
